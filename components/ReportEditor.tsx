...
const renderFormattedText = (text: string) => {
  ...
  return text.split('\n').map((line, i) => {
    ...
    return <div key={i} className="mb-1 min-h-[1.2em]">{formattedContent}</div>;
  });
};
...
const DraggableSignature: React.FC<{
  ...
  const handleMouseDown = (e: React.MouseEvent) => {
    e.preventDefault(); e.stopPropagation();
    dragStartRef.current = { x: e.clientX, y: e.clientY, initialX: style.x, initialY: style.y };
    const onMouseMove = (ev: MouseEvent) => {
      if (!dragStartRef.current) return;
      const dx = ev.clientX - dragStartRef.current.x;
      const dy = ev.clientY - dragStartRef.current.y;
      onUpdateSettings({ ...settings, signatureStyle: { ...style, x: dragStartRef.current.initialX + dx, y: dragStartRef.current.initialY + dy } });
    };
    const onMouseUp = () => { window.removeEventListener('mousemove', onMouseMove); window.removeEventListener('mouseup', onMouseUp); dragStartRef.current = null; };
    window.addEventListener('mousemove', onMouseMove); window.addEventListener('mouseup', onMouseUp);
  };
  const handleResizeMouseDown = (e: React.MouseEvent) => {
    ...
    const onMouseMove = (ev: MouseEvent) => {
      ...
      onUpdateSettings({ ...settings, signatureStyle: { ...style, width: newWidth } });
    };
    const onMouseUp = () => { window.removeEventListener('mousemove', onMouseMove); window.removeEventListener('mouseup', onMouseUp); resizeStartRef.current = null; };
    window.addEventListener('mousemove', onMouseMove); window.addEventListener('mouseup', onMouseUp);
  };
  ...
  return (
    <div className="relative group inline-block" style={{ transform: `translate(${style.x}px, ${style.y}px)`, width: `${style.width}px`, cursor: 'move', zIndex: 20, mixBlendMode: 'multiply' }} onMouseDown={handleMouseDown} title="Arraste para mover">
      ...
      <div className="absolute -bottom-1 -right-1 w-5 h-5 bg-blue-600 rounded-full cursor-nwse-resize opacity-0 group-hover:opacity-100 transition-opacity no-print flex items-center justify-center shadow-md z-30" onMouseDown={handleResizeMouseDown} title="Redimensionar"><div className="w-2 h-2 border-r-2 border-b-2 border-white"></div></div>
    </div>
  );
};
...
const ReportEditor: React.FC<ReportEditorProps> = ({ settings, patient, setPatient, report, setReport, capturedImages, onRemoveImage, onUpdateImage, onAddImageAfter, onOpenSettings, onNewReport, onUpdateSettings, onOpenUserManual }) => {
  ...
  const renderField = (label: string, value: string | undefined, onChange: (val: string) => void, placeholder: string = "", colSpan: string = "col-span-12") => (
    <div className={colSpan}>
      ...
      <input type="text" value={value || ''} onChange={e => onChange(e.target.value)} className="w-full text-slate-900 bg-transparent border-b border-slate-300 focus:border-blue-500 outline-none pb-0.5 text-sm print:hidden" placeholder={placeholder} />
      ...
      <div id="report-content" ref={reportContentRef} className="w-full max-w-5xl h-auto min-h-full p-8 md:p-12 print:p-0 print:w-full print:max-w-none print:shadow-none print:m-0 print-full-width print:pointer-events-auto">
        ...
        <div className="mb-8 print:mb-2 group/section">
          <div className="flex justify-between items-center mb-2.5">
            <div className="flex items-center gap-2.5">
              <div className="bg-slate-100 p-1.5 rounded-lg text-slate-600 group-hover/section:bg-blue-50 group-hover/section:text-blue-600 transition-colors"><FileText size={18} /></div>
        ...
        <div className="mb-6 print:mb-2 group/section">
          ...
          <div className={`relative border-2 rounded-xl overflow-hidden transition-all duration-300 ${isListening === 'conclusion' ? 'border-red-400 ring-4 ring-red-50' : 'border-slate-200 focus-within:border-indigo-500 focus-within:ring-4 focus-within:ring-indigo-50/50 focus-within:shadow-sm'} bg-slate-50/50 print:bg-white print:border-none print:p-0`}>
            ...
            <div className="hidden print:block p-0 text-sm leading-relaxed font-bold text-slate-800 min-h-[1em]">{renderFormattedText(report.conclusion)}</div>
          </div>
        </div>
                    ...
                    {regularImages.map((img, index) => (
                      <div key={img.id} className="relative h-auto group flex flex-col gap-1">
                        <div className="relative">
                          ...
                          {enhancingImageId === img.id && (<div className="absolute inset-0 bg-black/60 z-30 flex flex-col items-center justify-center rounded-sm backdrop-blur-[1px] text-white"><Loader2 size={32} className="animate-spin mb-2" /><span className="text-[10px] font-bold uppercase tracking-wider">Processando...</span></div>)}
                          {img.isAiEnhanced && (<div className="absolute top-2 left-2 bg-white/90 p-1 rounded-full shadow-sm z-10 pointer-events-none border border-indigo-100"><GeminiIcon size={16} /></div>)}
                          <button onClick={(e) => { e.stopPropagation(); onRemoveImage(img.id); }} className="absolute top-1 right-1 bg-red-600 text-white p-1 rounded-full opacity-0 group-hover:opacity-100 transition-opacity z-10 shadow-sm no-print"><X size={12} /></button>
                          <button onClick={(e) => { e.stopPropagation(); handleEnhanceImage(img.id, img.url); }} className="absolute bottom-1 right-1 bg-indigo-600 text-white p-1.5 rounded-full shadow-lg hover:bg-indigo-500 transition-all z-20 active:scale-95 group-hover:opacity-100 opacity-0 duration-200 no-print" disabled={enhancingImageId === img.id}><Sparkles size={14} /></button>
                        </div>
                        <div className="flex items-center gap-2 mt-1 px-1"><span className="text-[10px] font-bold text-slate-500 uppercase shrink-0">Foto {String(index + 1).padStart(2, '0')}</span><input type="text" value={img.caption || ''} onChange={(e) => onUpdateImage(img.id, { caption: e.target.value })} placeholder="Legenda..." className="flex-1 text-[10px] text-slate-700 bg-transparent border-b border-transparent hover:border-slate-200 focus:border-blue-400 outline-none w-full placeholder:text-slate-300 print:placeholder:text-transparent" /></div>
                      </div>
                    ))}
                  </div>
                  {hasAiImages && (<div className="mt-4 flex items-center justify-end gap-2 text-[10px] text-slate-500 italic border-t border-slate-100 pt-2"><GeminiIcon size={12} className="opacity-75" /><span>AI enhanced</span></div>)}
                </div>
              )}
              {qrCodeUrl && (
